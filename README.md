...
YANGモデル（`iida-network-model`）に準拠したネットワークトポロジをYAML形式で定義し、LLM (OpenAI / Azure OpenAI / Anthropic / Ollama) を通じて Graphviz の構成図（PNG / SVG）に変換するツールです。

生成した図を評価し、スコアが閾値に達するまで自律的に改善するループ構造を持ちます。
...
ノードのアイコンは d2v に**組み込みのデバイスアイコン画像**として同梱しています。ノードのタイプ（`device-type`）ごとに以下のアイコンが自動で割り当てられます。
...
YANGモデルに従ってトポロジをYAML形式でを記述します。

YANGモデル定義: [`yang/iida-network-model.yang`](yang/iida-network-model.yang)
...
### PowerPoint で描く際のガイドライン

draw.io や PowerPoint で図を描いて v2d に渡す場合、以下のルールを守ると認識精度が上がります。

#### ノード（機器の箱）

| ルール | ポイント |
|---|---|
| **1 箱 = 1 機器**、箱をはっきり閉じる | 箱の外縁が曖昧だとノードと認識されない |
| ホスト名を箱の中に書く | `hostname` フィールドの主ソース |
| 機器種別がわかる語をホスト名に含める | `rtr`, `sw`, `fw`, `srv`, `lb` → device_type 自動判定 |
| 管理 IP (loopback) を箱内に書く場合は `/32` 付きで | `loopback: 10.0.0.1/32` として抽出される |
| ポート横の IP は「ポート名 → IP」の順で箱の外に書く | `interfaces[].ip_address` として抽出される |

機器アイコン（絵文字）を使う場合の対応表：
...
| ルール | ポイント |
|---|---|
| 線の**両端**にポート名（`Gi0/1` 等）を書く | `source_port` / `target_port` に対応 |
| セグメント IP は線の**中央**に書く | `segment: 10.1.0.0/30` として抽出される |
| 外部・境界への接続は**破線**にする | 境界スタブとして扱われ `style: dashed` になる |
| 線を交差させない／折れ曲がりを少なくする | 交差するとどの線がどのノードにつながるか誤認されやすい |

#### ゾーン（クラスタ枠）

| ルール | ポイント |
|---|---|
| 背景色付きの**閉じた矩形**で機器をまとめる | クラスタ認識の主な手がかり |
| 枠の**見出し**（上端または左上）にゾーン名を書く | `clusters[].label` として抽出される |
| 枠と機器の箱の間に余白を取る | 機器がゾーン外と誤認されるのを防ぐ |

#### 全体

- **フォント 12pt 以上**、背景は白か薄い単色にする（画像は長辺 2048px に縮小される）
- ラベルを重ねない（ポート名とセグメント IP が混同される）
- 凡例・説明テキストは図から離す（notes 扱いになり誤認の原因になることがある）
- **優先度の高い最低限**: ①箱にホスト名（種別を含む語）②線中央にセグメント IP③ゾーンは背景色付き矩形＋見出し名
