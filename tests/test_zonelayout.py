...
def test_constraint_dot_intra_tier_chain() -> None:
    devices = [
        _dev("d-hub", "zone-hub"),
        _dev("d-a", "zone-a"),
        _dev("d-b", "zone-b"),
    ...
    # 同段に 2 ゾーン以上ある段では、不可視チェイン（->）を含む rank=same が出る。
    multi = [t for t in placement.tiers if len(t) >= 2]
    if multi:
        assert any("rank=same" in ln and "->" in ln and "style=invis" in ln for ln in lines)
