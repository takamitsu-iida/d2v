...
def _overview_text(
    ...
    lines.append(
        ...
        "各ゾーンを 1 つのまとまり（ノードまたはクラスタ）として描画してください。"
...
def _detail_text(
    ...
    node_total = len(devices) + len(external_ifaces)
    conn_total = len(intra) + len(boundary)
    ...
    for did, ifaces in external_ifaces.items():
        ext_dev = model.device_map.get(did, {"device-id": did})
        ext_zone = model.zone_of(did) or "unknown"
        lines.extend(
            parser.device_lines(ext_dev, only_interfaces=ifaces, external_zone=ext_zone)
        )
