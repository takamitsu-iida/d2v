...
次に **VS Code のウィンドウで `editor/` フォルダをルートとして開きます**
（ファイル > フォルダーを開く… で `editor/` を選択。リポジトリ全体ではなく
`editor/` 単体を開くのがポイントです）。

そのウィンドウで **F5** を押すと、この拡張を読み込んだ新しい VS Code ウィンドウ
（＝「拡張開発ホスト」）が別途起動します。F5 は通常デバッグ用のキーですが、
`editor/.vscode/launch.json` に「Run Extension」設定があるため、拡張の開発起動に
割り当てられています。デバッグ実行ビュー（Ctrl+Shift+D）上部で **「Run Extension」**
を選んで緑の再生ボタンを押しても同じです。

> 初回は F5 前に自動でビルド（`npm: compile`）が走ります。ビルドエラーがあると
> 拡張ホストが起動しないので、その場合は「問題」パネル（Ctrl+Shift+M）を確認して
> ください。

起動した**拡張開発ホストのウィンドウ側で**、`iida-network-model` の YAML
（例: `examples/sample_topology_small.yaml`）を開き、コマンドパレット
（Ctrl+Shift+P）から **「d2v: フォーカスプレビューを開く」** を実行します。
YAML ファイルを開いているときは、エディタ右上のタイトルバーのボタンからも
起動できます。

> 忘れずに、事前に別ターミナルで `python main.py serve` を起動しておいてください
> （「前提」の項を参照）。サーバが動いていないとプレビューは表示されません。
...
| `d2v.hops` | `1` | 何ホップ先まで表示するか（0～3。0 で指定ノードのみ） |
