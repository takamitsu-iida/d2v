...
- **ノード（デバイス）**: 箱で表現された機器。ホスト名・種別・管理IP・所属ゾーンを読み取る。ポート名の横に IP が書かれていれば `interfaces` に格納する。
  ...
  "nodes": [
    {
      ...
      "interfaces": [
        {"id": "Gi0/1", "ip_address": "192.168.1.1/24"}
      ],
...
- **インターフェース IP**: ポート名の近くに `192.168.1.1/24` のような IP が書かれている場合、そのノードの `interfaces` に `{"id": "ポート名", "ip_address": "IP"}` として格納する。ノード全体の管理 IP（`loopback`）やリンク中央のセグメント IP（`edges[].segment`）とは別物として扱うこと。
